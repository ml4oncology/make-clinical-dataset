...
def test_combine_features_mock(tmp_path):
    ...
    lab.to_parquet(f'{tmp_path}/interim/lab.parquet', compression='zstd', index=False)
    ...
    trt.to_parquet(f'{tmp_path}/interim/treatment.parquet', compression='zstd', index=False)
    ...
    dmg.to_parquet(f'{tmp_path}/interim/demographic.parquet', compression='zstd', index=False)
    ...
    sym.to_parquet(f'{tmp_path}/interim/symptom.parquet', compression='zstd', index=False)

    erv = pd.DataFrame([[mrn, pd.Timestamp('2020-01-01'), 10, 11, 12]], columns=['mrn', 'event_date', 'feat10', 'feat11', 'feat12'])
    erv.to_parquet(f'{tmp_path}/interim/emergency_room_visit.parquet', compression='zstd', index=False)
    ...
    last_seen.to_parquet(f'{tmp_path}/interim/last_seen_dates.parquet', compression='zstd')

    cmd = ['python', 'epr/scripts/unify.py', '--data-dir', tmp_path, '--output-dir', tmp_path]
...
def test_combine_features(tmp_path):
    """NOTE: tmp_path is an inbuilt Pytest fixture and pathlib.Path` object"""
    cmd = ['python', 'epr/scripts/unify.py', '--output-dir', tmp_path]
    result = subprocess.run(cmd, capture_output=True)
    assert result.returncode == 0